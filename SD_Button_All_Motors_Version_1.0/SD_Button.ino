...
void errorButtonBlink_cb(lv_timer_t *timer) {
    if (errorManager.hasErrors() && !errorManager.areErrorsAcknowledged()) {
        // Blink only when there are unacknowledged errors
        ...
        // Solid orange when errors exist but are acknowledged
        lv_obj_set_style_bg_color(objects.errors_button, 
                                 lv_palette_main(LV_PALETTE_ORANGE), LV_PART_MAIN);
    } else {
        // Blue when no errors
...
void showSdLogOnSettings(lv_event_t *e) {
    ...
    lv_label_set_text(objects.sd_data_logging, buffer);
...
if (!SD.begin(cs, spiSD, 4000000)) {
    ...
    sdCardReady = false;
    errorManager.logError(ERR_SD_INIT_FAILED, 0, "SD Card initialization failed");
} else {
...
// Find the correct label on errors_page for displaying errors
// You may need to adjust this based on your UI structure
...
lv_obj_add_event_cb(objects.sd_data_logging, duration_textarea_cb, LV_EVENT_READY, NULL);
